...
| `stick <at> <normal>` | the hit point, piped; the normal, by name from `collide` | `row.pos`, `row.stuck` | Land the row: position `at + normal · row.size` — resting TANGENT to the surface (ruling 24: a disc centred on the hit point sat half inside the plate, and a light row 6 µm inside it lit the underside) — and `row.stuck` set. A stuck row has no velocity — the sweep drops whatever the kernel added, every tick, which is what `stuck` means — so it stays where it landed; it still ages and still reads its curves. It rests at the radius it had when it landed: a stuck row's segment is zero length, so `collide` never fires again, and a shrinking ember keeps its landing height (recorded below). `collide \| stick` is the ember on the plate and the spark on the trim in one breath. |
...
| a stuck row re-rested as its radius changes (today it keeps its landing height) | the plate re-take shows the shrinking ember floating |
