...
**G6 — Budget, not clock.** The scheduler consumes a budget in row-steps per tick from a knob, never milliseconds. Under a spawn burst that exceeds the budget, `drift/<@em>/throttled` occurs and the tick still replays byte-identically. Mutation: read wall-clock anywhere in the sim; `repro-input` differs across two runs. **Green and bitten in spindrift's harness, beat 4** — two sprays, one knob, the dumps AND the plane transcript compared across two runs (a clock that leaked into the `throttled` payload survived a dump-only gate); the engine's G6 rides matryoshka's half of the beat.
