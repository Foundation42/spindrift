...
**G6 — Budget, not clock.** The scheduler consumes a budget in row-steps per tick from a knob, never milliseconds. Under a spawn burst that exceeds the budget, `drift/<@em>/throttled` occurs and the tick still replays byte-identically. Mutation: read wall-clock anywhere in the sim; `repro-input` differs across two runs. **Green and bitten in spindrift's harness, beat 4** — two sprays, one knob, the dumps AND the plane transcript compared across two runs (a clock that leaked into the `throttled` payload survived a dump-only gate); and in the engine (matryoshka `d50b100`): two sprays under 40 row-steps, the transcript hash compared first, the clock-in-the-priority mutation failing on it. **Green and bitten, both halves, beat 4.**
...
(`write` per write-verbs rev 3, ratified: the mode word rides the verb and the `plane.mod.…` path spelling is retired — a lane is `write <path> … add|mul`, never a second path. `above`, `kick`, `step`, `every`, `also` unchanged. In matryoshka the interim `.mul` lane on `rate` and `speed` folded exactly this until write-verbs beat 1 landed the real lanes and deleted it — matryoshka `352943d`, beside beat 4; §7.12.)
...
- particle lights are capped per spray in v1 (4, ruled; built, P3b — the brightest four by Oklab L, ties by row id). Many near-tied contributors is exactly the rank-swap condition (facet 4) — Mercury's light ring, but moving. Recorded, not solved — and met on the first capture: a two-ray shadow budget under four near-tied coals lets the fourth through the plate (beat 4's ruling 5)
...
**P4 — Tracer verbs and budget.** `collide`/`ground` on the CPU twin tracer, `stick`, the row-steps budget and priority scheduler, the governor writing the knob; G6. *Ruled order (beat 3 accepted):* the World caller — `collide` (the row's segment against the static tree through the CPU twin tracer: hit t, normal, material) and `ground` (nearest surface below), host words in matryoshka's registry (§7.7), the mock floor kept, a kernel naming a word the host lacks refused at mount, both exact at the row (the host's float query once, fixed-point results across the boundary); `stick` (pos ← hit, vel ← 0, `row.stuck` set; a stuck row still ages and reads its curves), read aloud before naming; the budget — `drift/budget/row_steps` read once per tick, over budget sprays updated by priority from fed-time-only inputs (staleness, then frustum from `plane.camera`, then dynamic-object intersection), the rest carried over with `drift/@name/throttled` as a MAILBOX occurrence (the spawn-refusal count keeps its meter under another word), a wall-clock governor writing the knob on the transcript; G6 — a burst over budget produces `throttled`, the tick replays byte-identically, and a coarsened-and-throttled run replays too (mutation: any wall-clock read in the sim path; repro-input differs); captures — spirit3 sparks landing on the trim, test_scene embers landing on the plate. **Beat 4, accepted** (`docs/cc-report-beat4.md`; rulings 20–27). **Then P3b as a half-beat:** the light appearance with the cap of 4, dust2's motes as its capture — *built, matryoshka `6f22cc4`, `c416d50`, reported in beat 4's report: the motes do not vanish at 8 mm, half do by distance; a quarter-pixel threshold brings them back at the ruling's cost; neither built.* Still fenced: trails, sub-sprays, per-row casts, blending, dynamic-prim collision (Ironwood's rain is the trigger).

**P5 — the keep (the closing beat).** Triggers met and the last customer scenes, ruled at beat 4's acceptance: (a) P3c — stochastic coverage (ruling 25; built `1f2368e`); sub-chunk leaves or rows sorted within a chunk (trigger met on dust2's 16 ms; built `5a820c0` — a leaf per run of 16 rows by median split, pixel-identical, 17.27 → 3.64 ms); the resting offset (ruling 24) with the plate re-taken from above and beneath; the applet's per-spray selector (trigger fired); dust2 and the plate re-captured. (b) `spray bind` following an entity, and Ironwood's torch sparks bound to `@torch` — the first spray that moves with what it is bound to. (c) tiltyard's fountain against the quintain — prices the editor gaps, nothing else. (d) The campaign close-out report: gates G0–G6 with their bites, every customer capture, the fence as it stands, the deferred fills with triggers, and G7 restated as the next campaign's opening gate with what this one paid for it — integer rows, the row plane, the host-side float boundary crossed once per tick, and replay on the transcript. Small: `spray_gate.py` loads dust2 once and runs every pose against it. **Still fenced, now for the next campaign:** the GPU evaluator (G7), trails via loop-loft, sub-sprays, per-row casts, dynamic-prim collision, blending. *Beside the beat:* `over` went home to rill's core (rill `23ac55c`, then `529e7d8` taking spindrift's two edge rulings — a life ≤ 0 refused, a far-past-life read clamped; another session, the same day) — same spelling, same bits, on the plane too; spindrift's kernel is deleted, the words this library registers are four plus the three tracer words.
...
20. *(beat 4 accepted)* **Segment versus move: (A), kept and said.** `collide` tests the kernel-start move; a row found inside a solid is placed on the face it came through; the discrepancy is a·dt², stated with its two numbers (0.0007 cells at 60 Hz, 0.1 cells on a 100 ms tick) in `drift-words.md` and here. (B) — integrate with the kernel-start velocity — is recorded with its trigger (a tunnel at frame rate in a customer scene), and (B) is explicit Euler where today is semi-implicit, so it would cost stability as well as hashes. (C) — one word reading the velocity as it will be — is refused; the snapshot rule is not broken for one word. (C′) recorded with the same trigger: a kernel may carry a second snapshot after an `integrate` line — statements below it read the post-move row — which is (C) without breaking the rule.
21. *(beat 4 accepted)* **Every spray verb that moves the population is fed input and rides the transcript.** `spray burst` under `--exec-at` goes on it; a record/replay pair with a burst must be like-for-like, gated.
22. *(beat 4 accepted)* **The three engine defaults ratified:** the budget's default is the picture's ceiling; the governor is off by default (a capture is never a function of the machine); the applet's `throttled` meter shows the latest occurrence, with the copy as written.
23. *(beat 4 accepted)* **Material +1 acceptable**, with the manual saying what it is: an opaque host handle, 0 is nothing, compared only against a value the host publishes, never against the engine's table index and never arithmetic. Material by name stays the trigger.
24. *(beat 4 accepted; first ruling withdrawn on Christian's screenshots, re-ruled)* **The resting offset.** The warm pool under the plate was not the shadow budget: a stuck row's position was the hit point, so a disc of radius `size` centred there sat half inside the surface (half-discs on the grass), and a light row 6 µm inside the plate lit its underside. `stick` places the row at `at + normal · row.size`, so a disc rests tangent to the surface; light rows inherit it. `collide` already pipes the normal; no new data — and rill's pipe now carries a producer's other outputs to the consumer's like-named ports, which is how the normal reaches `stick`. Landing gates assert the row sits ABOVE the surface by exactly its radius, on the floor and on the box; dumps and captures move once and are re-frozen. The plate is re-taken from above and from beneath: the shadow under the plate must be back and the grass must show whole discs. No blocker is built; if a warm pool remains after the offset, the capture comes back and THEN the shadow budget is looked at.
25. *(beat 4 accepted)* **Coverage.** A threshold, if kept, is a coverage knob on the kind, not a second appearance (a second disc with one number changed is a decoration). But the motes answered ruling 18 — 0.5 loses the far half, 0.25 makes hard specks claiming 2–4× their area — so the threshold is the workaround and the trigger for the fill is met. **The fill is stochastic coverage under facet 1:** a sub-pixel disc is a hit with probability equal to its analytic coverage against the cone footprint, from the frame's hashed sample, else the ray continues — the cutout path with a fractional test. Antialiasing, not transparency ordering: no sort, no OIT, deterministic under the existing hashed sampling, converges under accumulation. Fenced blending stays fenced. Built as P3c with dust2's motes as the capture; the threshold knob is not built. *Built, matryoshka `1f2368e` (beat 5): a disc narrower than the footprint samples its coverage from the frame's hash, a disc that spans it keeps the rim test — the column fills (9 602 mote pixels against 3 093), the ember and spark pairs stay bit-identical; the rim question (the pure rule for every disc) asked in beat 5's report.*
