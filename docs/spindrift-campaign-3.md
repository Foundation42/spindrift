...
Rulings in §7 are Christian's; everything else is proposed-and-seconded and proceeds unless overruled. Gates come first because that is what gets built. **The six rulings of §7 ratified as proposed 2026-09-05**; P10 opened the same evening. **CLOSED 2026-09-06** (`docs/cc-report-campaign3-close.md`): P10–P12 landed, G12 whole across nine scenes, the four non-lab pairs re-taken as the composite's; the puff is the next campaign's opening beat. Campaigns 1 and 2 stand (`docs/spindrift-campaign.md`, `docs/spindrift-campaign-2.md`); their rulings apply here unchanged, ruling 30 above all: test_scene is the lab, one pair per beat, the whole set once at the close.
...
**G17 — Lights without a cap.** A light row splats a deferred diffuse light onto the G-buffer within its range — N·L, distance falloff, the row's colour, the surface's albedo — into the composite, uncapped, no shadow; sixteen coals light the plate where four did, and the rank-swap seam of campaign 1 is gone by construction. **A light behind the camera still lights the frame** (Christian, 2026-09-05, on P10's pictures): the splat is keyed by the light's range on screen, not by where the row is — a light whose sphere holds the near plane covers the whole frame — so a coal a metre behind the eye lights the plate in front of it. Gate: the coals rig with one coal placed behind the camera; the plate is lit by it, and stood down it is not. Mutations: the splat ignoring the G-buffer normal (the underside of the plate lights up — the warm pool of ruling 24, for the last time); lights culled by their screen position (the coal behind the eye lights nothing).
...
| **P12 — the look** | `blend add` for sparks; `streak` along velocity in the vertex; the kind table's last two columns; the sparks rig on the plate — **and the shadow leak folded in** (Christian, 2026-09-06, on P11's smoke: the card takes the tracer's sun visibility of the surface behind it pixel for pixel, so a shadow pattern behind the plume shows through it): one visibility per card, sampled at the card's centre, so nothing can pattern inside a card; a sun depth pass over the TRACED geometry recorded as the true fix, trigger: a card whose whole area should be half in shadow | (extends G14; G16 re-gated on the leak) | matryoshka — *landed 2026-09-06, `1d6f223`: runs by blend in the sorted order, the streak an ellipse along the velocity, one visibility per card; the sparks pair new* |
| **close** | the whole set re-captured once — dust2's motes, oa_spirit3's sparks, the tiltyard — with the before/after in the report; the tracer's leaf gates kept, its captures retired | G12 whole | matryoshka — *done 2026-09-06, `202dc51`: nine references unmoved, four pairs re-taken; two bares moved by the engine's own history (dust2's BC7 re-freeze, a q3 floor decal), said in the close report* |
...
**Recorded on P10's pictures (Christian, 2026-09-05), with triggers:** *sprites in reflections without `traced`* — the reflection pass samples the composited sprite layer in screen space where the reflected ray's hit projects into the frame (the planar case, which a wet street is; the punk teardown's ground does exactly this), and falls back to nothing off-screen; trigger: the first reflective floor under a spray. `traced` stays the true mirror.
