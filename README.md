...
| `src/words.zig` | `spawn`, `gravity`, `perish`, `hear`, `relax`, `near`, `push`, `sync`, `infect`, `align`, `deposit` — row words registered into rill (`over`, the fifth from beat 3, is rill's core word since rill `23ac55c`); `collide`, `ground`, `slide`, `stick` — the TRACER table a host with a World registers (`stick` lands the row at the contact and stores `row.normal`; the appearance draws it at pos + normal × size; the normal rides the pipe by name; `slide` takes the normal OUT of the velocity and leaves the tangent, so a row runs along what it hit — `collide | slide | stick`) |
