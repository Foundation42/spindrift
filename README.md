...
zig build run -- --words                    # the row words, grouped by tag, with each tag's sentence
...
| `src/words.zig` | `spawn`, `gravity`, `perish`, `hear`, `relax`, `near`, `push`, `sync`, `infect`, `align`, `deposit` — row words registered into rill (`over`, the fifth from beat 3, is rill's core word since rill `23ac55c`); `collide`, `ground`, `slide`, `stick` — the TRACER table a host with a World registers (`collide` sweeps the row as a SPHERE of row.size and answers the CENTRE at contact, so `stick` lands the row one radius off the surface and stores `row.normal`, and the appearance draws it at pos, flat; the normal rides the pipe by name; `slide` takes the normal OUT of the velocity and leaves the tangent, so a row runs along what it hit — `collide | slide | stick`) |
| `src/scheduler.zig` | the row-steps budget over sprays: `plan` by staleness, frustum, dynamic, index — the first always runs |
| `kernels/embers.rill`, `kernels/smoke.rill`, `kernels/fire.rill`, `kernels/hearth.rill`, `kernels/motes.rill` | the kernels; embers is `drift-run`'s default. `fire` writes no colour — it writes a point in an APPEARANCE MANIFOLD (`row.u0`–`u2`: cooled, sooted, thinned) that a renderer reads an RBF set at, so a landed ember quenches to soot and a flying one blows thin, from one file and one kernel. `hearth` is the same manifold on a SLOPE: the ember runs down it instead of stopping, quenching the whole way, gated on `slate.contact` because a sliding row never sets `row.stuck`. `soot` is funideas §8, the smallest true version: `deposit $soot row.stuck` and a fire leaves a stain where it lands — 4.59 under the fire, 0.56 a metre out, 0.00 at three, and the mark outlives the row. A `cast` is one aggregate at the centre of the cloud, restated; a deposit is many marks where the rows actually were. `boids` is the flocking trio in three lines: separation is `push`, cohesion is `push` with a NEGATIVE gain, alignment is `align`, and the two radii come from two `near` lines because a consumer reads whichever one is nearest above it. Drive the cohesion gain across zero from a rill and the flock scatters and regroups, which is one number and not a mode. `kindle` is funideas §6's OTHER neighbour word — `infect` spreading a channel from whoever near has most of it, against a `relax 0` that heals, so the two rates are an epidemic threshold you can watch cross a cloud. `motes` is funideas §6: every row a lone oscillator with its own frequency and a whisper of coupling to whoever is in reach — nothing in it makes a wave, and a field of them makes travelling ones |
| `docs/drift-words.md` | the words manual, parity-gated both ways — including "The tags", where every row word says what it is FOR (five minted here, six borrowed from rill; the first tag is the home) |
